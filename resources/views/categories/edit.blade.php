...
@section('content')
    <x-common.page-breadcrumb pageTitle="Sửa danh mục" />

    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Sửa: {{ $category->name }}</h3>
            </div>
            ...
            <form action="{{ route('categories.update', $category) }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <x-forms.input label="Tên danh mục" name="name" :value="old('name', $category->name)" required />
                </div>

                <div>
                    <x-forms.input label="Slug" name="slug" :value="old('slug', $category->slug)" required />
                </div>
                ...
                <div>
                    ...
                    <select name="status"
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                        <option value="active" @selected(old('status', $category->status) === 'active')>Hoạt động</option>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">{{ old('description', $category->description) }}</textarea>
                </div>
